export default function BottomSheet({ open, onClose, title, children, zClassName = 'z-50' }) {
  return (
    ...
    >
      ...
      {/* Panel */}
      <div
        className={`relative bg-surface-raised rounded-t-2xl shadow-xl transition-transform duration-300 ${open ? 'translate-y-0' : 'translate-y-full'}`}
      >
        {/* Handle */}
        ...
        {title && (
          <div className="px-4 py-3 border-b border-line flex items-center justify-between">
            ...
            <button
              onClick={onClose}
              className="w-8 h-8 rounded-full bg-surface-warm flex items-center justify-center text-muted text-xl leading-none cursor-pointer hover:bg-surface-warm-hover"
              aria-label="Close"
            >
              ×
            </button>
